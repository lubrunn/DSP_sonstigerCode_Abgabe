...
#%%
def df_cleaner(df, lang = None, lang_controller = False):        
    ...
    # drop with missing in long lat
    df = df.dropna(subset = ["long", "lat"])
    # only select needed columns
    df = df[["id", "date","lat", "long", 
             "language", "retweets_count", 
             "likes_count"]]
...
for lang in lang_folders:
    ...
    for date in dates_missing:
        ...
        print(f"Proess took {time.time() - time1}")